...
class DIAYNAgent(DDPGAgent):
    ...
    def compute_intr_reward(self, skill, obs_diff, step):
        z_hat = torch.argmax(skill, dim=1)
        d_pred = self.diayn(obs_diff)
    ...
    def compute_diayn_loss(self, obs_diff, skill):
        """
        DF Loss
        """
        z_hat = torch.argmax(skill, dim=1)
    ...
    def update(self, replay_iter, step):
        ...
        if self.reward_free:
            ...
            metrics.update(self.update_diayn(skill, obs_diff, step))

            with torch.no_grad():
                # intr_reward = self.compute_intr_reward(skill, next_obs, step)
                intr_reward = self.compute_intr_reward(skill, obs_diff, step)
