...
class Workspace:
    ...
    def eval(self):
        for skill_num in range(self.agent.skill_dim + 1):
            step, episode, total_reward = 0, 0, 0
            time_step = self.eval_env.reset()
            self.video_recorder.init(self.eval_env, enabled=(episode == 0))

            if skill_num < self.agent.skill_dim:
                meta = self.agent.init_meta(skill_num)
            else:
                meta = self.agent.init_meta(-1)

            while not time_step.last():
                if (skill_num == self.agent.skill_dim) and (step % self.agent.update_skill_every_step == 0):
                    meta = self.agent.init_meta(-1)
...
def main(cfg):
    from extract_skill import Workspace as W
    root_dir = Path.cwd()
    workspace = W(cfg)
